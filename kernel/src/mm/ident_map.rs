...
impl<Arch: MemoryManagementArch, F: FrameAllocator> IdentPageMapper<Arch, F> {
    ...
    pub unsafe fn map_phys(
        ...
        let flags = EntryFlags::from_data(
            Arch::ENTRY_FLAG_DEFAULT_TABLE | Arch::ENTRY_FLAG_READWRITE | Arch::ENTRY_FLAG_EXEC,
        );
...
pub fn ident_pt_alloc() -> usize {
    let new_imapper: IdentPageMapper<X86_64MMArch, LockedFrameAllocator> =
        unsafe { IdentPageMapper::create(LockedFrameAllocator) };
    new_imapper.paddr().data()
}

pub fn ident_map(map_paddr: usize, virt: usize, phys: usize) {
    unsafe {
        IdentPageMapper::<X86_64MMArch, LockedFrameAllocator>::map_phys(
            PhysAddr::new(map_paddr),
            VirtAddr::new(virt),
            PhysAddr::new(phys),
