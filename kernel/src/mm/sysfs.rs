...
/// `/sys/firmware/memmap`的kset
static mut SYS_FIRMWARE_MEMMAP_KSET_INSTANCE: Option<Arc<KSet>> = None;

#[inline(always)]
#[allow(dead_code)]
pub fn sys_firmware_memmap_kset() -> Arc<KSet> {
    unsafe { SYS_FIRMWARE_MEMMAP_KSET_INSTANCE.clone().unwrap() }
}
...
impl MemmapDesc {
    ...
    pub fn inner(&self) -> SpinLockGuard<MemmapDescInner> {
...
impl Attribute for AttrType {
    ...
    fn show(&self, kobj: Arc<dyn KObject>, buf: &mut [u8]) -> Result<usize, SystemError> {
        ...
        match mt {
            1 => {
                let type_string = format!("System RAM\n");
                let bytes = type_string.as_bytes();
                buf[..bytes.len()].copy_from_slice(bytes);
                Ok(bytes.len())
            }
            2 => {
                let type_string = format!("Reserved\n");
                let bytes = type_string.as_bytes();
                buf[..bytes.len()].copy_from_slice(bytes);
                Ok(bytes.len())
            }
            3 => {
                let type_string = format!("ACPI Tables\n");
                let bytes = type_string.as_bytes();
                buf[..bytes.len()].copy_from_slice(bytes);
                Ok(bytes.len())
            }
...
pub fn early_memmap_init() {
    let mut manager = MemmapDescManager::new();
...
fn memmap_sysfs_init() -> Result<(), SystemError> {
    ...
    let memmap_kset = KSet::new("memmap".to_string());
    memmap_kset
        .register(Some(sys_firmware_kset()))
        .expect("register memmap kset failed");
    unsafe {
        SYS_FIRMWARE_MEMMAP_KSET_INSTANCE = Some(memmap_kset);
    }
...
fn memmap_sysfs_add(index: &usize, desc: &Arc<MemmapDesc>) {
    if unsafe { SYS_FIRMWARE_MEMMAP_KSET_INSTANCE.is_none() } {
        return;
    }

    let kset = sys_firmware_memmap_kset();
