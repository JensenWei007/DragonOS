use crate::arch::ipc::signal::SigCode;
...
impl Syscall for SysPidfdSendSignalHandle {
    ...
    fn handle(&self, args: &[usize], _frame: &mut TrapFrame) -> Result<usize, SystemError> {
        ...
        if sig == Signal::INVALID {
            warn!("Pidfd_Send_Signal: Send empty sig(0)");
            // 这里的信号是 0, 是空信号值, 其他的信号处理是怎样的不清楚, 但是这里应该直接返回成功, 因为 0 是空信号
            return Ok(0);
        }

        // 应该从参数获取
        let mut info = SigInfo::new(sig, 0, SigCode::User, SigType::Kill(Pid::new(pid as usize)));

        let ret = sig
            .send_signal_info(Some(&mut info), Pid::new(pid as usize))
            .map(|x| x as usize);
